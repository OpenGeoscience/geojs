...
window.geo = geo, geo.renderers = {}, geo.features = {}, geo.fileReaders = {}, geo.inherit = function(C, P) {
    "use strict";
    var F = inherit.func();
    F.prototype = P.prototype, C.prototype = new F(), C.prototype.constructor = C;
}, geo.inherit.func = function() {
    "use strict";
    return function() {};
}, window.inherit = geo.inherit, geo.registerFileReader = function(name, func) {
    "use strict";
    ...
    geo.util = {
        pointInPolygon: function(point, outer, inner) {
            var inside = !1, n = outer.length;
            return 3 > n ? !1 : (outer.forEach(function(vert, i) {
                var j = (n + i - 1) % n, intersect = outer[i].y > point.y != outer[j].y > point.y && point.x < (outer[j].x - outer[i].x) * (point.y - outer[i].y) / (outer[j].y - outer[i].y) + outer[i].x;
        ...
        convertColor: function(color) {
            return void 0 !== color.r && void 0 !== color.g && void 0 !== color.b ? color : ("string" == typeof color && (geo.util.cssColors.hasOwnProperty(color) ? color = geo.util.cssColors[color] : "#" === color.charAt(0) && (color = parseInt(color.slice(1), 16))), 
    ...
    return this.onIdle = function(handler) {
        ...
        if (void 0 === event && (m_eventHandlers = {}, m_idleHandlers = [], m_deferredCount = 0), 
        Array.isArray(event)) return event.forEach(function(e) {
            m_this.geoOff(e, arg);
        }), m_this;
        if (arg) {
    ...
    var m_this = this, m_parent = null, m_children = [], s_exit = this._exit, s_trigger = this.geoTrigger, s_addDeferred = this.addDeferred, s_onIdle = this.onIdle;
    return this.addDeferred = function(defer) {
        ...
        m_this.children = [], delete m_this.parent, s_exit();
    }, this;
...
geo.ellipsoid.UNIT_SPHERE = vgl.freezeObject(geo.ellipsoid(1, 1, 1)), geo.mercator = {
    ...
    var m_this = this, s_exit = this._exit, m_style = void 0 === arg.style ? {
        opacity: .5,
        color: [ .8, .8, .8 ],
        visible: !0,
        bin: 100
    } : arg.style, m_id = void 0 === arg.id ? geo.layer.newLayerId() : arg.id, m_name = "", m_gcs = "EPSG:4326", m_timeRange = null, m_source = arg.source || null, m_map = void 0 === arg.map ? null : arg.map, m_isReference = !1, m_x = 0, m_y = 0, m_width = 0, m_height = 0, m_node = null, m_canvas = null, m_renderer = null, m_initialized = !1, m_rendererName = void 0 === arg.renderer ? "vgl" : arg.renderer, m_dataTime = geo.timestamp(), m_updateTime = geo.timestamp(), m_drawTime = geo.timestamp(), m_sticky = void 0 === arg.sticky ? !0 : arg.sticky, m_active = void 0 === arg.active ? !0 : arg.active;
    return this.sticky = function() {
        ...
        m_renderer._exit(), m_node.off(), m_node.remove(), m_node = null, arg = {}, m_canvas = null, 
        m_renderer = null, s_exit();
    ...
    "use strict";
    if (spec = spec || {}, spec.type = "feature", "feature" !== spec.type) return console.warn("Unsupported layer type"), 
    null;
    if (spec.renderer = spec.renderer || "vgl", "d3" !== spec.renderer && "vgl" !== spec.renderer) return console.warn("Invalid renderer"), 
    null;
    var layer = map.createLayer(spec.type, spec);
    return layer ? (spec.features.forEach(function(f) {
        f.data = f.data || spec.data, f.feature = geo.feature.create(layer, f);
    }), layer) : (console.warn("Unable to create a layer"), null);
    ...
    var m_this = this, m_features = [], s_init = this._init, s_exit = this._exit, s_update = this._update, s_draw = this.draw;
    return this.createFeature = function(featureName, arg) {
        ...
        var i;
        for (i = 0; i < m_features.length; i += 1) m_features[i] === feature && (m_features[i]._exit(), 
        m_this.dataTime().modified(), m_this.modified(), m_features.splice(i, 1));
        ...
        var i;
        if (!m_features.length) return m_this;
        for (i = 0; i < m_features.length; i += 1) m_features[i]._exit(), m_this.removeChild(m_features[i]);
...
geo.event = {}, geo.event.layerAdd = "geo_layerAdd", geo.event.layerRemove = "geo_layerRemove", 
geo.event.zoom = "geo_zoom", geo.event.pan = "geo_pan", geo.event.resize = "geo_resize", 
geo.event.draw = "geo_draw", geo.event.drawEnd = "geo_drawEnd", geo.event.mousemove = "geo_mousemove", 
geo.event.mouseclick = "geo_mouseclick", geo.event.brush = "geo_brush", geo.event.brushend = "geo_brushend", 
geo.event.brushstart = "geo_brushstart", geo.event.transitionstart = "geo_transitionstart", 
geo.event.transitionend = "geo_transitionend", geo.event.clock = {
    play: "geo_clock_play",
    stop: "geo_clock_stop",
    ...
    function modifyVelocity(v, deltaT) {
        deltaT = 0 >= deltaT ? 30 : deltaT;
        var sf = springForce(), speed = calcSpeed(v), vx = v.x / speed, vy = v.y / speed;
        return speed *= Math.exp(-m_options.momentum.drag * deltaT), calcSpeed(sf) * deltaT + speed < m_options.momentum.minSpeed ? null : (speed > 0 ? (vx *= speed, 
        vy *= speed) : (vx = 0, vy = 0), {
            x: vx - sf.x * deltaT,
            y: vy - sf.y * deltaT
        });
    ...
    function springForce() {
        ...
        if (!m_options.spring.enabled) return {
            x: 0,
            y: 0
        };
        var ul = m_this.map().gcsToDisplay({
            x: -180,
            y: 82
        }), lr = m_this.map().gcsToDisplay({
            x: 180,
            y: -82
        }), c = m_options.spring.springConstant, width = m_this.map().node().width(), height = m_this.map().node().height();
        return xplus = c * Math.max(0, ul.x), xminus = c * Math.max(0, width - lr.x), yplus = c * Math.max(0, ul.y) / 2, 
    ...
    return m_options = $.extend(!0, {
        ...
        var out;
        return out = action ? m_state.action === action : !!m_state.action, out && (m_state = {}), 
        out;
        ...
        oldAction = m_state.action, m_state = {}, m_options.momentum.enabled && "pan" === oldAction && m_this.springBack(!0);
        ...
        "momentum" !== m_state.action && (initialVelocity || (m_mouse.velocity = {
            x: 0,
            y: 0
        }), m_state.action = "momentum", m_state.origin = m_this.mouse(), m_state.start = new Date(), 
        m_state.handler = function() {
            var v, s, last, dt;
            if (dt = Math.min(m_mouse.deltaTime, 30), "momentum" === m_state.action && m_this.map() && !m_this.map().transition()) {
                if (last = m_state.start.valueOf(), m_state.start = new Date(), v = modifyVelocity(m_mouse.velocity, m_state.start - last), 
                !v) return void (m_state = {});
                s = calcSpeed(v), s > m_options.momentum.maxSpeed && (s = m_options.momentum.maxSpeed / s, 
                v.x = v.x * s, v.y = v.y * s), isFinite(v.x) && isFinite(v.y) || (v.x = 0, v.y = 0), 
                m_mouse.velocity.x = v.x, m_mouse.velocity.y = v.y, m_this.map().pan({
                    x: m_mouse.velocity.x * dt,
                    y: m_mouse.velocity.y * dt
                }), m_state.handler && window.requestAnimationFrame(m_state.handler);
            }
        }, m_state.handler && window.requestAnimationFrame(m_state.handler));
    ...
    m_style = $.extend({
        ...
        fillOpacity: 1
    }, m_style), geo.fileReader.call(this, arg), this.canRead = function(file) {
        ...
        [ {
            ...
            y: coordinates[1],
            z: elv
        } ]) : (Array.isArray(coordinates[0][0]) && (coordinates = coordinates[0]), coordinates.map(function(c) {
            return {
                x: c[0],
                y: c[1],
                z: c[2]
        ...
        function _done(object) {
            ...
            features = m_this._featureArray(object), features.forEach(function(feature) {
                ...
                type ? "line" === type ? (style.fill = style.fill || !1, allFeatures.push(m_this._addFeature(type, [ coordinates ], style, feature.properties))) : "point" === type ? (style.stroke = style.stroke || !1, 
                allFeatures.push(m_this._addFeature(type, coordinates, style, feature.properties))) : "polygon" === type && (style.fill = void 0 === style.fill ? !0 : style.fill, 
                style.fillOpacity = void 0 === style.fillOpacity ? .25 : style.fillOpacity, allFeatures.push(m_this._addFeature("line", [ coordinates ], style, feature.properties))) : console.log("unsupported feature type: " + feature.geometry.type);
        ...
        var _style = $.extend({}, m_style, style), feature = m_this.layer().createFeature(type).data(m_this._buildData(coordinates, properties, style)).style(_style);
        return "line" === type ? feature.line(function(d) {
...
geo.map = function(arg) {
    ...
    var m_this = this, s_exit = this._exit, m_x = 0, m_y = 0, m_node = $(arg.node), m_width = arg.width || m_node.width(), m_height = arg.height || m_node.height(), m_gcs = void 0 === arg.gcs ? "EPSG:4326" : arg.gcs, m_uigcs = void 0 === arg.uigcs ? "EPSG:4326" : arg.uigcs, m_center = {
        x: 0,
        y: 0
    }, m_zoom = void 0 === arg.zoom ? 1 : arg.zoom, m_baseLayer = null, m_fileReader = null, m_interactor = null, m_validZoomRange = {
        min: 0,
        max: 16
    }, m_transition = null, m_queuedTransition = null, m_clock = null, m_bounds = {};
    return arg.center = geo.util.normalizeCoordinates(arg.center), arg.autoResize = void 0 === arg.autoResize ? !0 : arg.autoResize, 
    arg.clampBounds = void 0 === arg.clampBounds ? !0 : arg.clampBounds, this.gcs = function(arg) {
        ...
        }, base && base.renderer().geoTrigger(geo.event.zoom, evt, !0), recenter = evt.center, 
        evt.geo.preventDefault || (m_zoom = val, m_this._updateBounds(), m_this.children().forEach(function(child) {
            child.geoTrigger(geo.event.zoom, evt, !0);
        }), m_this.modified()), evt.center ? m_this.center(recenter) : m_this.pan({
            x: 0,
            y: 0
        }), m_this));
    }, this.pan = function(delta, force) {
        var evt, pt, corner1, corner2, base = m_this.baseLayer();
        return arg.clampBounds && !force && m_width && m_height && (pt = m_this.displayToGcs({
            x: delta.x,
            y: delta.y
            ...
            x: 180,
            y: -82
        }), delta.x = corner1.x > 0 && corner2.x < m_width ? (-corner1.x + m_width - corner2.x) / 2 : Math.max(Math.min(delta.x, -corner1.x), m_width - corner2.x), 
        delta.y = corner1.y > 0 && corner2.y < m_height ? (-corner1.y + m_height - corner2.y) / 2 : Math.max(Math.min(delta.y, -corner1.y), m_height - corner2.y)), 
        evt = {
            geo: {},
            screenDelta: delta,
            ...
            x: m_width / 2,
            y: m_height / 2
        }), m_this._updateBounds(), m_this.children().forEach(function(child) {
        ...
        return m_this.geoTrigger(geo.event.resize, {
            ...
            width: w,
            height: h
        }), m_this._updateBounds(), m_this.pan({
            x: 0,
            y: 0
        }), m_this.modified(), m_this;
        ...
        m_baseLayer = baseLayer, m_baseLayer.referenceLayer(!0), arg.center && m_this.center(arg.center, !0), 
        save = m_zoom, m_zoom = null, m_this.zoom(save), m_this._updateBounds(), m_this.pan({
            x: 0,
            y: 0
        }), m_this) : m_baseLayer;
    }, this.draw = function() {
        ...
        renderer || (renderer = "d3"), layer = m_this.createLayer("feature", {
            renderer: renderer
        ...
        m_this.node().off(".geo"), $(window).off("resize", resizeSelf), s_exit();
    }, this._init(arg), this.node().on("dragover.geo", function(e) {
        ...
        function anim(time) {
            var next, done = m_transition.done;
            if (next = m_queuedTransition, m_transition.start.time || (m_transition.start.time = time, 
            m_transition.end.time = time + defaultOpts.duration), m_transition.time = time - m_transition.start.time, 
            time >= m_transition.end.time || next) return next || (m_this.center(m_transition.end.center), 
            m_this.zoom(m_transition.end.zoom)), m_transition = null, m_this.geoTrigger(geo.event.transitionend, defaultOpts), 
            done && done(), void (next && (m_queuedTransition = null, m_this.transition(next)));
        ...
        if (void 0 === opts) return m_transition;
        if (m_transition) return m_queuedTransition = opts, m_this;
        var defaultOpts = {
        ...
        m_this.geoTrigger(geo.event.transitionstart, defaultOpts), defaultOpts.cancelNavigation ? (m_this.geoTrigger(geo.event.transitionend, defaultOpts), 
        m_this) : (defaultOpts.cancelAnimation ? (defaultOpts.duration = 0, anim(0)) : window.requestAnimationFrame(anim), 
        m_this);
    ...
    arg.autoResize && $(window).resize(resizeSelf), this;
}, geo.map.create = function(spec) {
    "use strict";
    var map = geo.map(spec);
    return map ? (spec.data = spec.data || [], spec.layers = spec.layers || [], spec.layers.forEach(function(l) {
        l.data = l.data || spec.data, l.layer = geo.layer.create(map, l);
    }), map) : (console.warn("Could not create map."), null);
    ...
    var m_this = this, s_exit = this._exit, m_selectionAPI = void 0 === arg.selectionAPI ? !1 : arg.selectionAPI, m_style = {}, m_layer = void 0 === arg.layer ? null : arg.layer, m_gcs = void 0 === arg.gcs ? "EPSG:4326" : arg.gcs, m_visible = void 0 === arg.visible ? !0 : arg.visible, m_bin = void 0 === arg.bin ? 0 : arg.bin, m_renderer = void 0 === arg.renderer ? null : arg.renderer, m_dataTime = geo.timestamp(), m_buildTime = geo.timestamp(), m_updateTime = geo.timestamp(), m_selectedFeatures = [];
    return this._bindMouseHandlers = function() {
        ...
        var mouse = m_this.layer().map().interactor().mouse(), data = m_this.data(), over = m_this.pointSearch(mouse.geo), newFeatures = [], oldFeatures = [], lastTop = -1, top = -1;
        m_selectedFeatures.length && (lastTop = m_selectedFeatures[m_selectedFeatures.length - 1]), 
        ...
        }), geo.feature.eventID += 1, newFeatures.forEach(function(i, idx) {
        ...
        }), geo.feature.eventID += 1, oldFeatures.forEach(function(i, idx) {
        ...
        }), geo.feature.eventID += 1, over.index.forEach(function(i, idx) {
        ...
        }), m_selectedFeatures = over.index, m_selectedFeatures.length && (top = m_selectedFeatures[m_selectedFeatures.length - 1]), 
        lastTop !== top && (-1 !== lastTop && m_this.geoTrigger(geo.event.feature.mouseoff, {
            data: data[lastTop],
            index: lastTop,
            mouse: mouse
        }, !0), -1 !== top && m_this.geoTrigger(geo.event.feature.mouseon, {
            data: data[top],
            index: top,
            mouse: mouse
        }, !0));
        ...
        geo.feature.eventID += 1, idx.forEach(function(i, idx) {
        ...
        return void 0 === arg1 ? m_style : "string" == typeof arg1 && void 0 === arg2 ? m_style[arg1] : void 0 === arg2 ? (m_style = $.extend({}, m_style, arg1), 
        ...
        return key.toLowerCase().match(/color$/) ? geo.util.isFunction(m_style[key]) ? (tmp = geo.util.ensureFunction(m_style[key]), 
        out = function() {
            return geo.util.convertColor(tmp.apply(this, arguments));
        }) : out = geo.util.ensureFunction(geo.util.convertColor(m_style[key])) : out = geo.util.ensureFunction(m_style[key]), 
        out;
    }, this.layer = function() {
        return m_layer;
    }, this.renderer = function() {
        ...
        return void 0 === data ? m_this.style("data") || [] : (m_this.style("data", data), 
        m_this.dataTime().modified(), m_this.modified(), m_this);
    ...
    this._exit = function() {
        m_this._unbindMouseHandlers(), m_selectedFeatures = [], m_style = {}, arg = {}, 
        s_exit();
    }, this._init(arg), this;
}, geo.feature.eventID = 0, geo.feature.create = function(layer, spec) {
    "use strict";
    var type = spec.type;
    if (!layer instanceof geo.layer) return console.warn("Invalid layer"), null;
    if ("object" != typeof spec) return console.warn("Invalid spec"), null;
    var feature = layer.createFeature(type);
    return feature ? (spec = spec || {}, spec.data = spec.data || [], feature.style(spec)) : (console.warn("Could not create feature type '" + type + "'"), 
    null);
    ...
    var m_this = this, s_init = this._init, m_rangeTree = null, s_data = this.data, s_style = this.style, m_maxRadius = 0;
    return this.position = function(val) {
        return void 0 === val ? m_this.style("position") : (m_this.style("position", val), 
        m_this.dataTime().modified(), m_this.modified(), m_this);
        ...
        return m_this.selectionAPI() ? (data = m_this.data(), data && data.length ? (map = m_this.layer().map(), 
        ...
        return void 0 === data ? s_data() : (s_data(data), m_this);
    }, this.style = function(arg1, arg2) {
        var val = s_style(arg1, arg2);
        return val === m_this && m_this.selectionAPI() && m_this._updateRangeTree(), val;
        ...
        void 0 !== arg.position && (defaultStyle.position = arg.position), m_this.style(defaultStyle), 
        m_this.dataTime().modified();
    }, m_this;
}, geo.event.pointFeature = $.extend({}, geo.event.feature), geo.pointFeature.create = function(layer, renderer, spec) {
    "use strict";
    return spec.type = "point", geo.feature.create(layer, spec);
    ...
    return this.line = function(val) {
        return void 0 === val ? m_this.style("line") : (m_this.style("line", val), m_this.dataTime().modified(), 
        m_this.modified(), m_this);
        ...
        void 0 !== arg.line && (defaultStyle.line = arg.line), void 0 !== arg.position && (defaultStyle.position = arg.position), 
        m_this.style(defaultStyle), m_this.dataTime().modified();
    }, this._init(arg), this;
}, geo.lineFeature.create = function(layer, spec) {
    "use strict";
    return spec.type = "line", geo.feature.create(layer, spec);
    ...
    function getCoordinates() {
        var posFunc = m_this.position(), polyFunc = m_this.polygon();
        m_coordinates = m_this.data().map(function(d, i) {
            var outer, inner, poly = polyFunc(d);
            return outer = (poly.outer || []).map(function(d0, j) {
    ...
    arg = arg || {}, geo.feature.call(this, arg);
    var m_position, m_polygon, m_this = this, s_init = this._init, s_data = this.data, m_coordinates = {
        outer: [],
        inner: []
    };
    return m_polygon = void 0 === arg.line ? function(d) {
        return d;
    } : arg.polygon, m_position = void 0 === arg.position ? function(d) {
        return d;
    } : arg.position, this.data = function(arg) {
        var ret = s_data(arg);
        return void 0 !== arg && getCoordinates(), ret;
    }, this.polygon = function(val) {
        return void 0 === val ? m_polygon : (m_polygon = val, m_this.dataTime().modified(), 
        m_this.modified(), getCoordinates(), m_this);
    }, this.position = function(val) {
        return void 0 === val ? m_position : (m_position = val, m_this.dataTime().modified(), 
        m_this.modified(), getCoordinates(), m_this);
    }, this.pointSearch = function(coordinate) {
        var found = [], indices = [], data = m_this.data();
        return m_coordinates.forEach(function(coord, i) {
            var inside = geo.util.pointInPolygon(coordinate, coord.outer, coord.inner);
            inside && (indices.push(i), found.push(data[i]));
        }), {
            index: indices,
            found: found
        };
...
}, inherit(geo.planeFeature, geo.polygonFeature), geo.vectorFeature = function(arg) {
    "use strict";
    if (!(this instanceof geo.vectorFeature)) return new geo.vectorFeature(arg);
    arg = arg || {}, geo.feature.call(this, arg);
    var m_this = this, s_init = this._init, s_style = this.style;
    this.origin = function(val) {
        return void 0 === val ? s_style("origin") : (s_style("origin", val), m_this.dataTime().modified(), 
        m_this.modified(), m_this);
    }, this.delta = function(val) {
        return void 0 === val ? s_style("delta") : (s_style("delta", val), m_this.dataTime().modified(), 
        m_this.modified(), m_this);
    }, this._init = function(arg) {
        s_init.call(m_this, arg);
        var defaultStyle = $.extend({}, {
        ...
        void 0 !== arg.origin && (defaultStyle.origin = arg.origin), m_this.style(defaultStyle), 
        m_this.dataTime().modified();
    };
}, inherit(geo.vectorFeature, geo.feature), geo.geomFeature = function(arg) {
    "use strict";
    ...
    return this._init = function(arg) {
        ...
        return m_this.data([]), m_links.forEach(function(link) {
            link._exit(), m_this.removeChild(link);
        }), m_links = [], m_points._exit(), m_this.removeChild(m_points), s_exit(), m_this;
    ...
    m_tileUrl = function(zoom, x, y) {
        ...
        !m_tiles[zoom][x][y]) {
            ...
            tile.INVALID = !1, tile.crossOrigin = "anonymous", tile.zoom = zoom, tile.index_x = x, 
            tile.index_y = y, tile.llx = llx, tile.lly = lly, tile.urx = urx, tile.ury = ury, 
            tile.lastused = new Date(), tile.src = m_tileUrl(zoom, x, Math.pow(2, zoom) - 1 - y), 
            m_tiles[zoom][x][y] = tile, m_pendingNewTiles.push(tile), m_numberOfCachedTiles += 1, 
            tile;
        }
    }, this._removeTiles = function() {
        var i, x, y, tile, zoom, currZoom = m_zoom, lastZoom = m_lastVisibleZoom;
        ...
        function tileOnLoad(tile) {
            ...
            return m_this.addDeferred(defer), function() {
                tile.INVALID || (tile.LOADING = !1, tile.LOADED = !0, (tile.REMOVING || tile.REMOVED) && tile.feature && tile.zoom !== m_zoom ? (tile.feature.bin(m_hiddenBinNumber), 
        ...
        worldDelta = Math.abs(worldPt2[0] - worldPt1[0]), worldDeltaY = Math.abs(worldPt2[1] - worldPt1[1]), 
        displayDelta = urx - llx, displayDeltaY = lly - ury, displayDeltaY > displayDelta && (displayDelta = displayDeltaY, 
        worldDelta = worldDeltaY), noOfTilesRequired = Math.round(displayDelta / 256), worldDeltaPerTile = worldDelta / noOfTilesRequired, 
        minDistWorldDeltaPerTile = Number.POSITIVE_INFINITY, i = 20; i >= 2; i -= 1) distWorldDeltaPerTile = Math.abs(360 / Math.pow(2, i) - worldDeltaPerTile), 
        minDistWorldDeltaPerTile > distWorldDeltaPerTile && (minDistWorldDeltaPerTile = distWorldDeltaPerTile, 
        m_zoom = i);
        for (tile1x = geo.mercator.long2tilex(worldPt1[0], m_zoom), tile1y = geo.mercator.lat2tiley(worldPt1[1], m_zoom), 
        tile2x = geo.mercator.long2tilex(worldPt2[0], m_zoom), tile2y = geo.mercator.lat2tiley(worldPt2[1], m_zoom), 
        tile1x = Math.max(tile1x, 0), tile1x = Math.min(Math.pow(2, m_zoom) - 1, tile1x), 
        tile1y = Math.max(tile1y, 0), tile1y = Math.min(Math.pow(2, m_zoom) - 1, tile1y), 
        tile2x = Math.max(tile2x, 0), tile2x = Math.min(Math.pow(2, m_zoom) - 1, tile2x), 
        tile2y = Math.max(tile2y, 0), tile2y = Math.min(Math.pow(2, m_zoom) - 1, tile2y), 
        tile1x > tile2x && (temp = tile1x, tile1x = tile2x, tile2x = temp), tile2y > tile1y && (temp = tile1y, 
        tile1y = tile2y, tile2y = temp), currStartX = tile1x, currEndX = tile2x, currStartY = Math.pow(2, m_zoom) - 1 - tile1y, 
        currEndY = Math.pow(2, m_zoom) - 1 - tile2y, currStartY > currEndY && (temp = currStartY, 
        ...
        lastStartY = lastEndY, lastEndY = temp), m_visibleTilesRange = {}, m_visibleTilesRange[m_zoom] = {
            startX: currStartX,
            endX: currEndX,
            startY: currStartY,
            endY: currEndY
            ...
            count: 0
        }, i = tile1x; tile2x >= i; i += 1) for (j = tile2y; tile1y >= j; j += 1) invJ = Math.pow(2, m_zoom) - 1 - j, 
        m_this._hasTile(m_zoom, i, invJ) ? (tile = m_tiles[m_zoom][i][invJ], tile.feature.bin(m_visibleBinNumber), 
        tile.LOADED && m_updateTimerId in m_pendingNewTilesStat && (m_pendingNewTilesStat[m_updateTimerId].count += 1), 
        tile.lastused = new Date(), tile.feature._update()) : tile = m_this._addTile(request, m_zoom, i, invJ), 
        ...
        }).origin([ tile.llx, tile.lly ]).upperLeft([ tile.llx, tile.ury ]).lowerRight([ tile.urx, tile.lly ]).gcs("EPSG:3857").style({
            image: tile,
        ...
        if ("/" !== baseUrl.charAt(m_baseUrl.length - 1) && (baseUrl += "/"), baseUrl !== m_baseUrl) {
            m_baseUrl = baseUrl;
            var tile, x, y, zoom;
            for (zoom in m_tiles) for (x in m_tiles[zoom]) for (y in m_tiles[zoom][x]) tile = m_tiles[zoom][x][y], 
            tile.INVALID = !0, m_this.deleteFeature(tile.feature);
            m_tiles = {}, m_pendingNewTiles = [], m_pendingInactiveTiles = [], m_numberOfCachedTiles = 0, 
            m_visibleTilesRange = {}, m_pendingNewTilesStat = {}, null !== m_updateTimerId && (clearTimeout(m_updateTimerId), 
            m_updateTimerId = null), this._update();
        }
    }, this.mapOpacity = function(val) {
        if (void 0 === val) return m_mapOpacity;
        if (val !== m_mapOpacity) {
            m_mapOpacity = val;
            var zoom, x, y, tile;
            for (zoom in m_tiles) for (x in m_tiles[zoom]) for (y in m_tiles[zoom][x]) tile = m_tiles[zoom][x][y], 
            tile.feature.style().opacity = val, tile.feature._update();
            m_this.modified();
        }
        return m_this;
    }, this._exit = function() {
        m_tiles = {}, m_pendingNewTiles = [], m_pendingInactiveTiles = [], m_numberOfCachedTiles = 0, 
        m_visibleTilesRange = {}, m_pendingNewTilesStat = {}, s_exit();
    }, this;
...
geo.gl = {}, geo.gl.renderer = function(arg) {
...
}, inherit(geo.gl.renderer, geo.renderer), geo.gl.lineFeature = function(arg) {
    "use strict";
    ...
    function createGLLines() {
        var i, j, k, v, len, lineItem, lineItemData, vertTemp, pos, posIdx3, posBuf, nextBuf, prevBuf, offsetBuf, indicesBuf, strokeWidthBuf, strokeColorBuf, strokeOpacityBuf, dest, dest3, data = m_this.data(), numSegments = 0, vert = [ {}, {} ], position = [], posFunc = m_this.position(), strkWidthFunc = m_this.style.get("strokeWidth"), strkColorFunc = m_this.style.get("strokeColor"), strkOpacityFunc = m_this.style.get("strokeOpacity"), order = m_this.featureVertices(), geom = m_mapper.geometryData();
        for (i = 0; i < data.length; i += 1) for (lineItem = m_this.line()(data[i], i), 
        numSegments += lineItem.length - 1, j = 0; j < lineItem.length; j += 1) pos = posFunc(lineItem[j], j, lineItem, i), 
        pos instanceof geo.latlng ? (position.push(pos.x()), position.push(pos.y()), position.push(0)) : (position.push(pos.x), 
        position.push(pos.y), position.push(pos.z || 0));
        for (position = geo.transform.transformCoordinates(m_this.gcs(), m_this.layer().map().gcs(), position, 3), 
        len = numSegments * order.length, posBuf = getBuffer(geom, "pos", 3 * len), nextBuf = getBuffer(geom, "next", 3 * len), 
        prevBuf = getBuffer(geom, "prev", 3 * len), offsetBuf = getBuffer(geom, "offset", 1 * len), 
        strokeWidthBuf = getBuffer(geom, "strokeWidth", 1 * len), strokeColorBuf = getBuffer(geom, "strokeColor", 3 * len), 
        strokeOpacityBuf = getBuffer(geom, "strokeOpacity", 1 * len), indicesBuf = geom.primitive(0).indices(), 
        indicesBuf instanceof Uint16Array && indicesBuf.length === len || (indicesBuf = new Uint16Array(len), 
        geom.primitive(0).setIndices(indicesBuf)), i = posIdx3 = dest = dest3 = 0; i < data.length; i += 1) for (lineItem = m_this.line()(data[i], i), 
        j = 0; j < lineItem.length; j += 1, posIdx3 += 3) if (lineItemData = lineItem[j], 
        j && (vertTemp = vert[0], vert[0] = vert[1], vert[1] = vertTemp), vert[1].pos = posIdx3, 
        vert[1].prev = posIdx3 - (j ? 3 : 0), vert[1].next = posIdx3 + (j + 1 < lineItem.length ? 3 : 0), 
        vert[1].strokeWidth = strkWidthFunc(lineItemData, j, lineItem, i), vert[1].strokeColor = strkColorFunc(lineItemData, j, lineItem, i), 
        vert[1].strokeOpacity = strkOpacityFunc(lineItemData, j, lineItem, i), j) for (k = 0; k < order.length; k += 1, 
        dest += 1, dest3 += 3) v = vert[order[k][0]], posBuf[dest3] = position[v.pos], posBuf[dest3 + 1] = position[v.pos + 1], 
        posBuf[dest3 + 2] = position[v.pos + 2], prevBuf[dest3] = position[v.prev], prevBuf[dest3 + 1] = position[v.prev + 1], 
        prevBuf[dest3 + 2] = position[v.prev + 2], nextBuf[dest3] = position[v.next], nextBuf[dest3 + 1] = position[v.next + 1], 
        nextBuf[dest3 + 2] = position[v.next + 2], offsetBuf[dest] = order[k][1], strokeWidthBuf[dest] = v.strokeWidth, 
        strokeColorBuf[dest3] = v.strokeColor.r, strokeColorBuf[dest3 + 1] = v.strokeColor.g, 
        strokeColorBuf[dest3 + 2] = v.strokeColor.b, strokeOpacityBuf[dest] = v.strokeOpacity;
        geom.boundsDirty(!0), m_mapper.modified(), m_mapper.boundsDirtyTimestamp().modified();
    }
    function getBuffer(geom, srcName, len) {
        var data, src = geom.sourceByName(srcName);
        return data = src.data(), data instanceof Float32Array && data.length === len ? data : (data = new Float32Array(len), 
    ...
    return this.featureVertices = function() {
        ...
        prog.addUniform(prjUnif), prog.addUniform(m_pixelWidthUnif), prog.addUniform(m_aspectUniform), 
        prog.addShader(fs), prog.addShader(vs), m_material.addAttribute(prog), m_material.addAttribute(vgl.blend()), 
        m_actor = vgl.actor(), m_actor.setMaterial(m_material), m_actor.setMapper(m_mapper), 
        geom.addSource(posData), geom.addSource(prvPosData), geom.addSource(nxtPosData), 
        geom.addSource(strkWidthData), geom.addSource(strkColorData), geom.addSource(strkOpacityData), 
        geom.addSource(offPosData), geom.addPrimitive(triangles), m_mapper.setGeometryData(geom);
        ...
        m_pixelWidthUnif.set(1 / m_this.renderer().width()), m_aspectUniform.set(m_this.renderer().width() / m_this.renderer().height()), 
        m_actor.setVisible(m_this.visible()), m_actor.material().setBinNumber(m_this.bin()), 
        m_this.updateTime().modified();
    }, this._exit = function() {
        m_this.renderer().contextRenderer().removeActor(m_actor), s_exit();
    }, this._init(arg), this;
...
geo.gl.pointFeature = function(arg) {
    ...
    function pointPolygon(x, y, w, h) {
        ...
        switch (m_primitiveShape) {
          case "triangle":
            verts = [ x, y - 2 * h, x - w * Math.sqrt(3), y + h, x + w * Math.sqrt(3), y + h ];
            break;
    ...
    function createGLPoints() {
        var i, j, posBuf, posVal, posFunc, unitBuf, indices, radius, radiusVal, radFunc, stroke, strokeVal, strokeFunc, strokeWidth, strokeWidthVal, strokeWidthFunc, strokeOpacity, strokeOpacityVal, strokeOpactityFunc, strokeColor, strokeColorVal, strokeColorFunc, fill, fillVal, fillFunc, fillOpacity, fillOpacityVal, fillOpacityFunc, fillColor, fillColorVal, fillColorFunc, item, ivpf, ivpf3, iunit, i3, numPts = m_this.data().length, unit = pointPolygon(0, 0, 1, 1), position = new Array(3 * numPts), vpf = m_this.verticesPerFeature(), data = m_this.data(), geom = m_mapper.geometryData();
        for (posFunc = m_this.position(), radFunc = m_this.style.get("radius"), strokeFunc = m_this.style.get("stroke"), 
        strokeWidthFunc = m_this.style.get("strokeWidth"), strokeOpactityFunc = m_this.style.get("strokeOpacity"), 
        strokeColorFunc = m_this.style.get("strokeColor"), fillFunc = m_this.style.get("fill"), 
        fillOpacityFunc = m_this.style.get("fillOpacity"), fillColorFunc = m_this.style.get("fillColor"), 
        i = i3 = 0; numPts > i; i += 1, i3 += 3) posVal = posFunc(data[i]), position[i3] = posVal.x, 
        position[i3 + 1] = posVal.y, position[i3 + 2] = posVal.z || 0;
        for (position = geo.transform.transformCoordinates(m_this.gcs(), m_this.layer().map().gcs(), position, 3), 
        posBuf = getBuffer(geom, "pos", vpf * numPts * 3), "sprite" !== m_primitiveShape && (unitBuf = getBuffer(geom, "unit", vpf * numPts * 2)), 
        radius = getBuffer(geom, "rad", vpf * numPts * 1), stroke = getBuffer(geom, "stroke", vpf * numPts * 1), 
        strokeWidth = getBuffer(geom, "strokeWidth", vpf * numPts * 1), strokeOpacity = getBuffer(geom, "strokeOpacity", vpf * numPts * 1), 
        strokeColor = getBuffer(geom, "strokeColor", vpf * numPts * 3), fill = getBuffer(geom, "fill", vpf * numPts * 1), 
        fillOpacity = getBuffer(geom, "fillOpacity", vpf * numPts * 1), fillColor = getBuffer(geom, "fillColor", vpf * numPts * 3), 
        indices = geom.primitive(0).indices(), indices instanceof Uint16Array && indices.length === vpf * numPts || (indices = new Uint16Array(vpf * numPts), 
        geom.primitive(0).setIndices(indices)), i = ivpf = ivpf3 = iunit = i3 = 0; numPts > i; i += 1, 
        i3 += 3) {
            if (item = data[i], "sprite" !== m_primitiveShape) for (j = 0; j < unit.length; j += 1, 
            iunit += 1) unitBuf[iunit] = unit[j];
            for (radiusVal = radFunc(item), strokeVal = strokeFunc(item) ? 1 : 0, strokeWidthVal = strokeWidthFunc(item), 
            strokeOpacityVal = strokeOpactityFunc(item), strokeColorVal = strokeColorFunc(item), 
            fillVal = fillFunc(item) ? 1 : 0, fillOpacityVal = fillOpacityFunc(item), fillColorVal = fillColorFunc(item), 
            j = 0; vpf > j; j += 1, ivpf += 1, ivpf3 += 3) posBuf[ivpf3] = position[i3], posBuf[ivpf3 + 1] = position[i3 + 1], 
            posBuf[ivpf3 + 2] = position[i3 + 2], radius[ivpf] = radiusVal, stroke[ivpf] = strokeVal, 
            strokeWidth[ivpf] = strokeWidthVal, strokeOpacity[ivpf] = strokeOpacityVal, strokeColor[ivpf3] = strokeColorVal.r, 
            strokeColor[ivpf3 + 1] = strokeColorVal.g, strokeColor[ivpf3 + 2] = strokeColorVal.b, 
            fill[ivpf] = fillVal, fillOpacity[ivpf] = fillOpacityVal, fillColor[ivpf3] = fillColorVal.r, 
            fillColor[ivpf3 + 1] = fillColorVal.g, fillColor[ivpf3 + 2] = fillColorVal.b;
        }
        geom.boundsDirty(!0), m_mapper.modified(), m_mapper.boundsDirtyTimestamp().modified();
    }
    function getBuffer(geom, srcName, len) {
    ...
    var m_this = this, s_exit = this._exit, m_actor = null, m_mapper = null, m_pixelWidthUniform = null, m_aspectUniform = null, m_dynamicDraw = void 0 === arg.dynamicDraw ? !1 : arg.dynamicDraw, m_primitiveShape = "sprite", s_init = this._init, s_update = this._update, vertexShaderSource = null, fragmentShaderSource = null;
    return ("triangle" === arg.primitiveShape || "square" === arg.primitiveShape || "sprite" === arg.primitiveShape) && (m_primitiveShape = arg.primitiveShape), 
    vertexShaderSource = [ "#ifdef GL_ES", "  precision highp float;", "#endif", "attribute vec3 pos;", "attribute float rad;", "attribute vec3 fillColor;", "attribute vec3 strokeColor;", "attribute float fillOpacity;", "attribute float strokeWidth;", "attribute float strokeOpacity;", "attribute float fill;", "attribute float stroke;", "uniform float pixelWidth;", "uniform float aspect;", "uniform mat4 modelViewMatrix;", "uniform mat4 projectionMatrix;", "varying vec4 fillColorVar;", "varying vec4 strokeColorVar;", "varying float radiusVar;", "varying float strokeWidthVar;", "varying float fillVar;", "varying float strokeVar;" ], 
    "sprite" !== m_primitiveShape && (vertexShaderSource = vertexShaderSource.concat([ "attribute vec2 unit;", "varying vec3 unitVar;" ])), 
    vertexShaderSource.push.apply(vertexShaderSource, [ "void main(void)", "{", "  strokeWidthVar = strokeWidth;", "  // No stroke or fill implies nothing to draw", "  if (stroke < 1.0 || strokeWidth <= 0.0 || strokeOpacity <= 0.0) {", "    strokeVar = 0.0;", "    strokeWidthVar = 0.0;", "  }", "  else", "    strokeVar = 1.0;", "  if (fill < 1.0 || rad <= 0.0 || fillOpacity <= 0.0)", "    fillVar = 0.0;", "  else", "    fillVar = 1.0;", "  if (fillVar == 0.0 && strokeVar == 0.0) {", "    gl_Position = vec4(2, 2, 0, 1);", "    return;", "  }", "  fillColorVar = vec4 (fillColor, fillOpacity);", "  strokeColorVar = vec4 (strokeColor, strokeOpacity);", "  radiusVar = rad;" ]), 
    "sprite" === m_primitiveShape ? vertexShaderSource.push.apply(vertexShaderSource, [ "  gl_Position = (projectionMatrix * modelViewMatrix * vec4(pos, 1.0)).xyzw;", "  gl_PointSize = 2.0 * (rad + strokeWidthVar); ", "}" ]) : vertexShaderSource.push.apply(vertexShaderSource, [ "  unitVar = vec3 (unit, 1.0);", "  vec4 p = (projectionMatrix * modelViewMatrix * vec4(pos, 1.0)).xyzw;", "  if (p.w != 0.0) {", "    p = p / p.w;", "  }", "  p += (rad + strokeWidthVar) * ", "       vec4 (unit.x * pixelWidth, unit.y * pixelWidth * aspect, 0.0, 1.0);", "  gl_Position = vec4(p.xyz, 1.0);", "}" ]), 
    vertexShaderSource = vertexShaderSource.join("\n"), fragmentShaderSource = [ "#ifdef GL_ES", "  precision highp float;", "#endif", "uniform float aspect;", "varying vec4 fillColorVar;", "varying vec4 strokeColorVar;", "varying float radiusVar;", "varying float strokeWidthVar;", "varying float fillVar;", "varying float strokeVar;" ], 
    "sprite" !== m_primitiveShape && fragmentShaderSource.push("varying vec3 unitVar;"), 
    fragmentShaderSource.push.apply(fragmentShaderSource, [ "void main () {", "  vec4 strokeColor, fillColor;", "  float endStep;", "  // No stroke or fill implies nothing to draw", "  if (fillVar == 0.0 && strokeVar == 0.0)", "    discard;" ]), 
    fragmentShaderSource.push("sprite" === m_primitiveShape ? "  float rad = 2.0 * length (gl_PointCoord - vec2(0.5));" : "  float rad = length (unitVar.xy);"), 
    fragmentShaderSource.push.apply(fragmentShaderSource, [ "  if (rad > 1.0)", "    discard;", "  // If there is no stroke, the fill region should transition to nothing", "  if (strokeVar == 0.0) {", "    strokeColor = vec4 (fillColorVar.rgb, 0.0);", "    endStep = 1.0;", "  } else {", "    strokeColor = strokeColorVar;", "    endStep = radiusVar / (radiusVar + strokeWidthVar);", "  }", "  // Likewise, if there is no fill, the stroke should transition to nothing", "  if (fillVar == 0.0)", "    fillColor = vec4 (strokeColor.rgb, 0.0);", "  else", "    fillColor = fillColorVar;", "  // Distance to antialias over", "  float antialiasDist = 3.0 / (2.0 * radiusVar);", "  if (rad < endStep) {", "    float step = smoothstep (endStep - antialiasDist, endStep, rad);", "    gl_FragColor = mix (fillColor, strokeColor, step);", "  } else {", "    float step = smoothstep (1.0 - antialiasDist, 1.0, rad);", "    gl_FragColor = mix (strokeColor, vec4 (strokeColor.rgb, 0.0), step);", "  }", "}" ]), 
    fragmentShaderSource = fragmentShaderSource.join("\n"), this.actors = function() {
        ...
        "sprite" === m_primitiveShape && (primitive = new vgl.points()), m_pixelWidthUniform = new vgl.floatUniform("pixelWidth", 2 / m_this.renderer().width()), 
        m_aspectUniform = new vgl.floatUniform("aspect", m_this.renderer().width() / m_this.renderer().height()), 
        s_init.call(m_this, arg), m_mapper = vgl.mapper({
            dynamicDraw: m_dynamicDraw
        }), prog.addVertexAttribute(posAttr, vgl.vertexAttributeKeys.Position), "sprite" !== m_primitiveShape && prog.addVertexAttribute(unitAttr, vgl.vertexAttributeKeysIndexed.One), 
        prog.addVertexAttribute(radAttr, vgl.vertexAttributeKeysIndexed.Two), prog.addVertexAttribute(strokeWidthAttr, vgl.vertexAttributeKeysIndexed.Three), 
        prog.addVertexAttribute(fillColorAttr, vgl.vertexAttributeKeysIndexed.Four), prog.addVertexAttribute(fillAttr, vgl.vertexAttributeKeysIndexed.Five), 
        prog.addVertexAttribute(strokeColorAttr, vgl.vertexAttributeKeysIndexed.Six), prog.addVertexAttribute(strokeAttr, vgl.vertexAttributeKeysIndexed.Seven), 
        ...
        m_actor.setMapper(m_mapper), geom.addSource(sourcePositions), geom.addSource(sourceUnits), 
        geom.addSource(sourceRadius), geom.addSource(sourceStrokeWidth), geom.addSource(sourceFillColor), 
        geom.addSource(sourceFill), geom.addSource(sourceStrokeColor), geom.addSource(sourceStroke), 
        geom.addSource(sourceAlpha), geom.addSource(sourceStrokeOpacity), geom.addPrimitive(primitive), 
...
geo.gl.geomFeature = function(arg) {
    ...
    var m_this = this, s_exit = this._exit, m_actor = null, m_onloadCallback = void 0 === arg.onload ? null : arg.onload;
    return this.coords = function() {
        ...
        m_actor.material().shaderProgram().uniform("opacity").set(void 0 !== m_this.style().opacity ? m_this.style().opacity : 1), 
        ...
        m_actor.material().setBinNumber(m_this.bin()), m_actor.material().shaderProgram().uniform("opacity").set(void 0 !== m_this.style().opacity ? m_this.style().opacity : 1)), 
        m_this.updateTime().modified();
    }, this._exit = function() {
        m_this.renderer().contextRenderer().removeActor(m_actor), s_exit();
    }, this;
...
geo.gl.polygonFeature = function(arg) {
    ...
    function createGLPolygons() {
        ...
        fillOpacityFunc = m_this.style.get("fillOpacity"), m_this.data().forEach(function(item) {
            for (polygon = m_this.polygon()(item, itemIndex), polygonItem = polygon.outer || [], 
            holes = polygon.inner || [], polygonItemCoordIndex = 0, extRing = [], extIndex = 0, 
            extLength = polygonItem.length - 1, extRing[0] = [], intIndex = 0, polygonItem.forEach(function(extRingCoords) {
                ...
                extRing[0].push(posInstance instanceof geo.latlng ? {
                    ...
                    y: posInstance.y,
                    i: fillColor.length
                }), fillColorInstance = fillColorFunc(extRingCoords, polygonItemCoordIndex, item, itemIndex), 
                fillColor.push([ fillColorInstance.r, fillColorInstance.g, fillColorInstance.b ]), 
                fillOpacity.push(fillOpacityFunc(extRingCoords, polygonItemCoordIndex, item, itemIndex)), 
                ...
                extRing[intIndex + 1] = [], hole.forEach(function(intRingCoords) {
                    posInstance = posFunc(intRingCoords, polygonItemCoordIndex, item, itemIndex), extRing[intIndex + 1].push(posInstance instanceof geo.latlng ? {
                        x: posInstance.x(),
                        y: posInstance.y(),
                        i: fillColor.length
                    } : {
                        ...
                        y: posInstance.y,
                        i: fillColor.length
                    }), fillColorInstance = fillColorFunc(intRingCoords, polygonItemCoordIndex, item, itemIndex), 
                    fillColor.push([ fillColorInstance.r, fillColorInstance.g, fillColorInstance.b ]), 
            ...
            newTriangList.forEach(function(polygonIndex) {
                ...
                position.push([ polygonItemCoords.x, polygonItemCoords.y, polygonItemCoords.z || 0 ]), 
                fillColorNew.push(fillColor[polygonItemCoords.i]), fillOpacityNew.push(fillOpacity[polygonItemCoords.i]);
            }), itemIndex += 1;
    ...
    var m_this = this, s_exit = this._exit, m_actor = vgl.actor(), m_mapper = vgl.mapper(), m_material = vgl.material(), s_init = this._init, s_update = this._update;
    return this._init = function(arg) {
...
geo.gl._vglViewerInstances = {
    ...
    "use strict";
    function makeViewer() {
        canvas = $(document.createElement("canvas")), canvas.attr("class", "webgl-canvas");
        var viewer = vgl.viewer(canvas.get(0));
        return viewer.renderWindow().removeRenderer(viewer.renderWindow().activeRenderer()), 
        viewer.init(), viewer;
    }
    var mapIdx, canvas, maps = geo.gl._vglViewerInstances.maps, viewers = geo.gl._vglViewerInstances.viewers;
    ...
    viewers[mapIdx]._exit = function() {
        canvas && (canvas.off(), canvas.remove());
    }, viewers[mapIdx];
    ...
    for (mapIdx = 0; mapIdx < viewers.length; mapIdx += 1) (void 0 === viewer || viewer === viewers[mapIdx]) && (viewer._exit(), 
    maps.splice(mapIdx, 1), viewers.splice(mapIdx, 1));
    ...
    var m_this = this, s_exit = this._exit, m_viewer = geo.gl.vglViewerInstance(this.layer().map()), m_contextRenderer = vgl.renderer(), m_width = 0, m_height = 0, s_init = this._init;
    return m_contextRenderer.setResetScene(!1), this.width = function() {
        ...
        geo.gl.vglViewerInstance.deleteCache(m_viewer), s_exit();
        ...
        var camera, renderWindow, center, dir, focusPoint, position, newZ, vglRenderer = m_this.contextRenderer(), layer = m_this.layer();
        ...
        position = camera.position(), newZ = 360 * Math.pow(2, -evt.zoomLevel), evt.pan = null, 
        evt.screenPosition && (center = renderWindow.displayToWorld(evt.screenPosition.x, evt.screenPosition.y, focusPoint, vglRenderer), 
...
}, inherit(geo.gl.vglRenderer, geo.gl.renderer), geo.registerRenderer("vgl", geo.gl.vglRenderer), 
geo.d3 = {}, function() {
    "use strict";
    ...
    var m_id = "d3-" + geo.d3.uniqueID(), s_exit = this._exit, m_this = this, s_draw = this.draw;
    return this._d3id = function() {
        ...
        m_this.renderer()._removeFeature(m_this._d3id()), s_exit();
    }, this;
...
geo.d3.vectorFeature = function(arg) {
    ...
    function updateMarkers(data, stroke, opacity) {
        var renderer = m_this.renderer(), sel = m_this.renderer()._definitions().selectAll("marker.geo-vector").data(data);
        sel.enter().append("marker").attr("id", markerID).attr("class", "geo-vector").attr("viewBox", "0 0 10 10").attr("refX", "1").attr("refY", "5").attr("markerWidth", "5").attr("markerHeight", "5").attr("orient", "auto").append("path").attr("d", "M 0 0 L 10 5 L 0 10 z"), 
        sel.exit().remove(), sel.style("stroke", renderer._convertColor(stroke)).style("fill", renderer._convertColor(stroke)).style("opacity", opacity);
    }
    if (!(this instanceof geo.d3.vectorFeature)) return new geo.d3.vectorFeature(arg);
    arg = arg || {}, geo.vectorFeature.call(this, arg), geo.d3.object.call(this);
    var m_sticky, m_this = this, s_init = this._init, s_exit = this._exit, s_update = this._update, m_buildTime = geo.timestamp(), m_style = {};
    return this._init = function(arg) {
        return s_init.call(m_this, arg), m_sticky = m_this.layer().sticky(), m_this;
    }, this._build = function() {
        function getScale() {
        ...
        var data = m_this.data(), s_style = m_this.style.get(), m_renderer = m_this.renderer(), orig_func = m_this.origin(), size_func = m_this.delta(), cache = [], scale = m_this.style("scale"), max = Number.NEGATIVE_INFINITY;
        return s_update.call(m_this), data || (data = []), cache = data.map(function(d, i) {
            var origin = m_renderer.worldToDisplay(orig_func(d, i)), delta = size_func(d, i);
            return max = Math.max(max, delta.x * delta.x + delta.y * delta.y), {
                x1: origin.x,
                y1: origin.y,
                dx: delta.x,
                dy: -delta.y
            };
        }), max = Math.sqrt(max), scale || (scale = 75 / max), m_style.id = m_this._d3id(), 
        m_style.data = data, m_style.append = "line", m_style.attributes = {
            x1: function(d, i) {
                return cache[i].x1;
            },
            y1: function(d, i) {
        ...
        }, m_style.classes = [ "d3VectorFeature" ], updateMarkers(data, s_style.strokeColor, s_style.strokeOpacity), 
        m_this.renderer()._drawFeatures(m_style), m_buildTime.modified(), m_this.updateTime().modified(), 
        m_this;
    }, this._update = function() {
        return s_update.call(m_this), m_this.getMTime() >= m_buildTime.getMTime() ? m_this._build() : updateMarkers(m_style.data, m_style.style.strokeColor, m_style.style.strokeOpacity), 
        m_this;
    }, this._exit = function() {
        s_exit.call(m_this), m_style = {}, updateMarkers([], null, null);
    }, this._init(arg), this;
}, inherit(geo.d3.vectorFeature, geo.vectorFeature), geo.registerFeature("d3", "vector", geo.d3.vectorFeature), 
geo.d3.d3Renderer = function(arg) {
    ...
    geo.d3.object.call(this, arg), arg = arg || {};
    ...
    return this._convertColor = function(f, g) {
        ...
        m_features = {}, m_this.canvas().remove(), s_exit();
...
}, inherit(geo.d3.d3Renderer, geo.renderer), geo.registerRenderer("d3", geo.d3.d3Renderer), 
geo.gui = {}, geo.gui.uiLayer = function(arg) {
    "use strict";
    if (arg.renderer = "d3", arg.sticky = !1, !(this instanceof geo.gui.uiLayer)) return new geo.gui.uiLayer(arg);
    geo.layer.call(this, arg);
    var m_this = this, s_exit = this._exit;
...
geo.gui.widget = function(arg) {
    ...
    var m_this = this, s_exit = this._exit, m_layer = arg.layer;
    this._init = function() {
        m_this.modified();
    }, this._exit = function() {
        m_this.children().forEach(function(child) {
        ...
        return m_this.removeChild(feature), feature._exit(), m_this;
    }, this.layer = function() {
        return m_layer;
    };
    ...
    var m_xscale, m_yscale, m_plus, m_minus, m_track, m_nub, m_plusIcon, m_minusIcon, m_group, m_lowContrast, m_this = this, s_exit = this._exit, m_width = 20, m_height = 100, m_nubSize = 10, m_highlightDur = 100;
    ...
    m_lowContrast = {
        ...
        m_group.remove(), m_this.layer().geoOff(geo.event.zoom), s_exit();
...
function($, geo, d3) {
    ...
    var load = function() {
        function isColorKey(key) {
            return "color" === key.slice(key.length - 5, key.length).toLowerCase();
        }
        function makeColorScale(data, acc) {
            ...
            if (!d3) return console.warn("d3 is unavailable, cannot apply color scales."), acc;
            var domain, cannotHandle = !1, doNotHandle = !0, categorical = !1, min = Number.POSITIVE_INFINITY, max = Number.NEGATIVE_INFINITY;
            return domain = geo.util.isFunction(acc) ? d3.set(data.map(acc)).values() : [ acc ], 
            domain.forEach(function(v) {
                ("string" != typeof v || "object" != typeof geo.util.convertColor(v)) && (doNotHandle = !1), 
                "string" == typeof v ? categorical = !0 : isFinite(v) ? +v > max ? max = +v : min > +v && (min = +v) : cannotHandle = !0;
            }), cannotHandle ? acc : doNotHandle ? acc : wrap(categorical ? domain.length <= 10 ? d3.scale.category10().domain(domain) : domain.length <= 20 ? d3.scale.category20().domain(domain) : d3.scale.category20().domain(domain) : d3.scale.linear().range([ "rgb(252,141,89)", "rgb(255,255,191)", "rgb(145,191,219)" ]).domain([ min, (min + max) / 2, max ]));
        }
        if (!$.widget) return void ($.fn.geojsMap = function() {
            throw new Error("The geojs jquery plugin requires jquery ui to be available.");
        });
        var initialized = !1;
        $.widget("geojs.geojsMap", {
            options: {
                center: {
                    latitude: 0,
                    longitude: 0
            ...
            _create: function() {
                !this._map && this.element.length && (initialized && (console.warn("Geojs already initialized in this window."), 
                delete window.gl), initialized = !0, this._map = geo.map({
                    width: this.options.width,
                    height: this.options.height,
                    zoom: this.options.zoom,
                    center: this.options.center,
                    node: this.element.get(0)
                }), this._baseLayer = this._map.createLayer(this.options.baseLayer, {
                    renderer: this.options.baseRenderer
                }), this._resize({
                    width: 800,
                    height: 600
                }), this._layers = [], this.update());
            },
            update: function(layers) {
                var m_this = this;
                return this.options.layers = layers || this.options.layers || [], this._layers.forEach(function(layer) {
                    layer.clear(), m_this._map.deleteLayer(layer);
                }), this._layers = this.options.layers.map(function(layer) {
                    return layer.data = layer.data || m_this.options.data, (layer.features || []).forEach(function(feature) {
                        var scl, data = feature.data || layer.data || [];
                        "point" === feature.type && (feature.size ? feature._size = feature.size : null === feature.size && delete feature._size, 
                        data.length && feature._size && (scl = d3.scale.linear().domain(d3.extent(data, feature._size)).range([ 5, 20 ]), 
                        feature.radius = function() {
                            return scl(feature._size.apply(this, arguments));
                        }), delete feature.size);
                        var key;
                        for (key in feature) feature.hasOwnProperty(key) && isColorKey(key) && (feature[key] = makeColorScale(data, feature[key]));
                    }), geo.layer.create(m_this._map, layer);
                }), this.redraw(), this;
            },
            _resize: function(size) {
                var width = this.options.width, height = this.options.height;
                size && (width = size.width, height = size.height), width || (width = this.element.width()), 
                height || (height = this.element.height()), this._map.resize(0, 0, width, height);
            },
            redraw: function() {
